...
SELECT DISTINCT *
,COUNT (*) OVER(PARTITION BY product_id ORDER BY customer_id ASC) as [number_of_purchases]		
FROM customer_purchases 
...
,total_sale
	...
	SELECT market_date
	,total_sale
	,ROW_NUMBER() OVER(ORDER BY total_sale DESC) AS max_sale
	FROM temp.total_sales) x
	WHERE x.max_sale = 1
...
,total_sale
	...
	SELECT market_date
	,total_sale
	,ROW_NUMBER() OVER(ORDER BY total_sale ASC) AS min_sale
	FROM temp.total_sales) x
	WHERE x.min_sale = 1
